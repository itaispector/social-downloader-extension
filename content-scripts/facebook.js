...
const BTN_ID = 'sdl-fb-download-btn';
let lastUrl = location.href;

injectExtractorScript();
init();

// Detect SPA navigation via title-element mutations (reliable on Facebook)
...
if (titleEl) {
  ...
  titleObserver.observe(document.head || document.documentElement, { childList: true, subtree: true });
}

function isVideoPage() {
  return /\/(watch|video|reel)(\/|\?|$)/.test(location.pathname + location.search);
}

function init() {
  if (!isVideoPage()) return;

  // Try multiple selectors for Facebook's ever-changing DOM
  waitForElement(
    [
      '[aria-label="Actions for this video"]',
      '[data-testid="video-player"]',
      'video',
    ],
    (el) => {
      // Walk up to find a good insertion point
      const anchor = el.closest('[role="main"]') || el.closest('[data-pagelet]') || el.parentElement;
...
function injectButton(anchor) {
  ...
  wrapper.id = BTN_ID;
  wrapper.className = 'sdl-download-btn sdl-fb-btn';
  ...
  // Insert at top of the anchor so it's visible near the video
...
async function onButtonClick(e) {
  ...
  try {
    [data, settings] = await Promise.all([
      requestExtraction('EXTRACT_FACEBOOK'),
      getSettings(),
    ]);
  } catch (err) {
  ...
  const modal = buildModal(data, settings.facebook || {});
  document.body.appendChild(modal);
  modal.getBoundingClientRect();
  positionModal(modal, document.getElementById(BTN_ID));
}

function buildModal(data, fbSettings) {
  const { videoQuality = 'hd' } = fbSettings;

  // Filter formats by quality preference
  let formats = data.formats;
  if (videoQuality === 'hd') {
    const hd = formats.filter((f) => f.quality === 'hd');
    if (hd.length) formats = hd;
  } else {
    const sd = formats.filter((f) => f.quality === 'sd');
    if (sd.length) formats = sd;
  }
  ...
  if (formats.length) {
    formats.forEach((f) => {
      const qualityLabel = f.quality === 'hd' ? 'HD' : 'SD';
      const btn = document.createElement('button');
      btn.className = 'sdl-download-option';
      btn.innerHTML = `
        <span class="sdl-option-label">Video · MP4</span>
        <span class="sdl-quality-badge">${qualityLabel}</span>
      `;
      btn.addEventListener('click', () => {
        removeModal();
        sendDownloadRequest(f.url, `${data.title}.mp4`, 'facebook').catch((err) => showError(err.message));
      });
    ...
    msg.textContent = 'No downloadable video found. The video may be private.';
